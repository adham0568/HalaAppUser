...
class SearchPage extends StatefulWidget {
  ...
  String Name;
  List Product;
  int whichPage;//hala ture//rust false
  SearchPage({required this.Product,required this.whichPage,required this.Name,required this.NamePrudact,Key? key}) : super(key: key);
...
class _SearchPageState extends State<SearchPage> {
  ...
  searchData({required String searchText}) {
    searchResult.clear(); // لمسح النتائج السابقة
    bool exactMatchFound = false;
    for (int i = 0; i < widget.Product.length; i++) {
      String productName = widget.Product[i]['Name'].toLowerCase();
      // إذا تم العثور على تطابق تام للحروف الأولى
      if (productName.startsWith(searchText.toLowerCase())) {
        searchResult.add(widget.Product[i]);
        exactMatchFound = true;
      }
      // إذا تم إدخال حرف آخر ولم يتم العثور على تطابق تام في الحرف الثاني
      else if (exactMatchFound && searchText.isNotEmpty &&
          !productName.startsWith(searchText.toLowerCase(), 1)) {
        break; // انقطاع البحث بمجرد عدم العثور على تطابق تام للحرف الثاني
      }
    }
  }
  ...
  void initState() {
    ...
    if(widget.whichPage==2){Search.text==widget.Name;searchData(searchText:widget.Name);}
    super.initState();
  }
  @override
                       ...
                       child:AnimationLimiter(
                         child: GridView.count(
                           ...
                           children: List.generate(
                             searchResult.length>0?searchResult.length:0, (int index) {
                                         ...
                                         children: [
                                           Stack(
                                             children: [
                                               ...
                                               searchResult[index]['Discount']*1.0>0?
                                               const Row(children: [
                                               ...
                                               Positioned(
                                                   ...
                                                   child:Container(decoration: BoxDecoration(color:Colors.black54,borderRadius: BorderRadius.circular(10)),
                                                       child: AddToCartWidget(Prudact:searchResult[index],
                                                         ColorIcon: Colors.pink,SizeIcon: 30,
                                                       ))
                                               )
                                             ],
                                           ),
                                           Text(searchResult[index]['Name'],style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 15),),
                                           ...
                                           Row(
                                             ...
                                             children: [
                                               ...
                                               //Text('${snapshot.data!.docs[index]['Prise']*1.0-snapshot.data!.docs[index]['Discount']*1.0}'+' ₪',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold,fontSize: 18),),
                                             ],
                                           )
                                               :
                                           Text('${searchResult[index]['Prise']*1.0} ₪',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold,fontSize: 18),),
                                         ],
                                       ),
                                     ),
                                   ),
          ...
          child: Column(
            children: [
              ...
              StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance.collection('AdminData').where('Location',isEqualTo:DataUser!.City)
